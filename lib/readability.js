...
var readability = {
   ...
   **/
  regexps: {
    unlikelyCandidates: /combx|comment|community|disqus|extra|foot|header|menu|remark|rss|shoutbox|sidebar|sponsor|ad-break|agegate|pagination|pager|popup|tweet|twitter|sharedaddy/i,
    okMaybeItsACandidate: /and|article|body|column|main|shadow/i,
    positive: /article|body|content|entry|hentry|main|page|pagination|post|text|blog|story/i,
    negative: /combx|comment|com-|contact|foot|footer|footnote|masthead|media|meta|outbrain|promo|related|scroll|shoutbox|sidebar|sponsor|shopping|tags|tool|widget|sharedaddy/i,
  ...
  embedAllowedRegexps: [
    /https?:\/\/(?:[a-z0-9\-]+\.)?(?:(?:dipdive|youtube|vimeo|viddler|dailymotion|flickr|viddy|spreecast|issuu|telly|trutv|socialcam|ted|rdio|nymag|youku|letv|qq|iqiyi|tudou|sina|56|qik|speakerdeck|mixcloud|bravotv|animoto|revision3|spotify|bandcamp|soundcloud|hulu|yahoo|keek)\.com|(?:slideshare)\.net|(?:wordpress|justin|blip)\.tv|(?:vid)\.ly)/i,
  ...
  init: function () {
    ...
    //custom filter morhod
    readability.removeSNS(document);
    ...
    readability.removeAuthorGravatar(document);
  ...
  removeScripts: function (doc) {
    ...
    for (var i = scripts.length - 1; i >= 0; i -= 1) {
      if ((typeof(scripts[i].src) === "undefined" || !scripts[i].src ) || ( scripts[i].src.indexOf('readability') == -1 && scripts[i].src.indexOf('typekit') == -1)) {
        scripts[i].nodeValue = "";
  ...
  /**
   * Removes SNS tags from the document.
   *
   * @param Element
   **/
  removeSNS: function (doc) {
    var weShareBtns = doc.getElementsByTagName('wb:share-button');
    for (var i = weShareBtns.length - 1; i >= 0; i -= 1) {
      if (weShareBtns[i].parentNode) {
        weShareBtns[i].parentNode.removeChild(weShareBtns[i]);
      }
    }
  },

  removeAuthorGravatar: function(doc){
    var gravatarIconTags = doc.getElementsByTagName('img');
    for (var i = gravatarIconTags.length - 1; i >= 0; i -= 1) {
      if(gravatarIconTags[i].src && (gravatarIconTags[i].src.indexOf('gravatar') != -1)){
        if (gravatarIconTags[i].parentNode) {
          gravatarIconTags[i].parentNode.removeChild(gravatarIconTags[i]);
        }
      }
    }
  },
  ...
  getClassWeight: function (e) {
    ...
    //console.log('find className: '+e.className);
...
exports.parse = function parse(theHtml, url, options, callback) {
  ...
  if (options.encoding && !{'utf8': 1}[options.encoding.toLowerCase()]) {
    var buff = iconv.decode(theHtml, options.encoding);
    theHtml = iconv.encode(buff, 'utf8');
  }
  ...
  try {
    ...
    win = doc.parentWindow || doc.createWindow(); //for backward compatibility with jsdom <= 0.1.20

    var sourceOnParseError = options.onParseError;
    options.onParseError = function () {
      win.close();
      sourceOnParseError && sourceOnParseError();
    };
