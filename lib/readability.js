   ...
   if(options.encoding && !{'utf8':1}[options.encoding.toLowerCase()]) {
	...
	try {
		...
		function createDocWithHTMLParser() {
            ...
            registerCall && registerCall('readability_win_create');
			return doc;
		}
		...
		function createDocWithHTML5() {
			...
			parser.parse(html);
            registerCall && registerCall('readability_win_create_html5');
			return doc;
		}
		...
		if (!doc.body) {
			...
			if (HTML5) {
                ...
                registerCall && registerCall('readability_win_close_before_html5');
				...
				if (doc.body) {
                    ...
                    registerCall && registerCall('readability_win_close_after_good_html5');
                    doc.parentWindow.close();

					doc = createDocWithHTMLParser();
				} else {
                    registerCall && registerCall('readability_win_close_after_bad_html5');
                    doc.parentWindow.close();
                }
			}
		}
		...
		if (!doc.body) {
            ...
            registerCall && registerCall('readability_win_close_still');
            doc.parentWindow.close();
        ...
        options.onParseError = function() {
            registerCall && registerCall('readability_win_close_error');
            win.close();
		...
		start(win, options, function(html) {
			...
			process.nextTick(function() {
                registerCall && registerCall('readability_win_close');
				win.close();
            });
		});
	} catch(e) {
		...
		if (win && win.close){
			process.nextTick(function() {
                registerCall && registerCall('readability_win_close_catch');
				win.close();
			});
		}
